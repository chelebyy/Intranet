    ...
    <aside className="flex flex-col w-64 bg-sidebar dark:bg-dark-sidebar border-r border-border-color dark:border-dark-border shrink-0 h-full transition-colors duration-200">
      <div className="flex flex-col gap-4 p-4 h-full">
        ...
        <nav className="flex flex-col gap-1 mt-4">
          <div onClick={() => { onNavigate(Page.DASHBOARD); navigate('/dashboard'); }} className={navItemClass(Page.DASHBOARD)}>
            <span className={iconClass(Page.DASHBOARD)}>dashboard</span>
            <p className="text-sm font-medium leading-normal">Dashboard</p>
          </div>

          <div onClick={() => { onNavigate(Page.USER_LIST); navigate('/users'); }} className={navItemClass(Page.USER_LIST)}>
            <span className={iconClass(Page.USER_LIST)}>group</span>
            <p className="text-sm font-medium leading-normal">Kullanıcı Yönetimi</p>
          </div>

          <div onClick={() => { onNavigate(Page.DEPARTMENT_LIST); navigate('/departments'); }} className={navItemClass(Page.DEPARTMENT_LIST)}>
          ...
          <div onClick={() => { onNavigate(Page.REPORTS); navigate('/reports'); }} className={navItemClass(Page.REPORTS)}>
            <span className={iconClass(Page.REPORTS)}>bar_chart</span>
            <p className="text-sm font-medium leading-normal">Raporlar</p>
          </div>

          <div onClick={() => { onNavigate(Page.ROLES_PERMISSIONS); navigate('/roles'); }} className={navItemClass(Page.ROLES_PERMISSIONS)}>
